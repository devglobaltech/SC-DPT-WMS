/****** Object:  StoredProcedure [dbo].[RPT_PREPICKING_MULTIPLE]    Script Date: 03/03/2015 12:27:04 ******/
...
BEGIN
	...
	DECLARE @FLG_NO_BEST_FIT VARCHAR(2)
	DECLARE @QTY_BEST_FIT	NUMERIC(20,5)
	DECLARE @QTY_BEST_FIT_A	NUMERIC(20,5)
	DECLARE @CANT_SOL		NUMERIC(20,5)
	DECLARE @SALDO			NUMERIC(20,5)
	...
	BEGIN TRY	
		...
		SET @CUR_VIAJES = CURSOR FOR 
			...
			FROM	#TMP_VIAJES T INNER JOIN SYS_INT_DOCUMENTO D 
					ON (D.CLIENTE_ID =  T.CLIENTE_ID  AND D.CODIGO_VIAJE = T.CODIGO_VIAJE  AND D.DOC_EXT = T.PEDIDO )
		...
		BEGIN
			----------------------------------------------------------------------------
			--CON ESTO OBTENGO SI EL CLIENTE TRABAJA CON PICKING DE PALLET COMPLETO.
			----------------------------------------------------------------------------
			SELECT	@FLG_NO_BEST_FIT=ISNULL(CP.FLG_PALLET_COMPLETO,'0')
			...
			BEGIN
				...
				SELECT	@NRO_LOTE = DD.NRO_LOTE, @NRO_PARTIDA = DD.NRO_PARTIDA, @NRO_SERIE = DD.PROP3, @PRODUCTO_ID = DD.PRODUCTO_ID,
						@EST_MERC_ID=DD.EST_MERC_ID, @CAT_LOG_ID=DD.CAT_LOG_ID, @CANT_SOL=DD.CANTIDAD_SOLICITADA 
				...
				FROM	(
						...
						WHERE	((@FLG_NO_BEST_FIT='0')OR(P.BESTFIT='0'))
						GROUP BY 
				...
				--BEST FIT.
				IF (@FLG_NO_BEST_FIT='1')
				BEGIN
					----------------------------------------------------------------------------------------------------------------------------------
					--CON ESTA QUERY OBTENGO TODO LO PICKEABLE DESDE EL ESTADO LOGICO Y ESTADO DE MERCADERIA, PERO QUE ESTA EN POSICIONES DE BEST FIT
					--LAS POSICIONES DE BEST FIT NO SE CRUZAN CON UBICACIONES DE PICKING. SOLO ES EL TOTAL, NO QUIERE DECIR QUE TODO ESTO SEA ASIGNABLE.
					----------------------------------------------------------------------------------------------------------------------------------					
					SELECT  @QTY_BEST_FIT=ISNULL(SUM(RL.CANTIDAD),0)
					FROM 	RL_DET_DOC_TRANS_POSICION RL
							INNER JOIN DET_DOCUMENTO_TRANSACCION DDT(NOLOCK)	ON(RL.DOC_TRANS_ID=DDT.DOC_TRANS_ID AND RL.NRO_LINEA_TRANS=DDT.NRO_LINEA_TRANS)
							INNER JOIN DET_DOCUMENTO DD				(NOLOCK)	ON (DDT.DOCUMENTO_ID=DD.DOCUMENTO_ID AND DDT.NRO_LINEA_DOC=DD.NRO_LINEA)
							INNER JOIN CATEGORIA_LOGICA CL			(NOLOCK)	ON (RL.CLIENTE_ID=CL.CLIENTE_ID AND RL.CAT_LOG_ID=CL.CAT_LOG_ID AND CL.DISP_EGRESO='1' AND CL.PICKING='1')
							INNER JOIN POSICION P					(NOLOCK)	ON (RL.POSICION_ACTUAL=P.POSICION_ID AND P.POS_LOCKEADA='0' AND P.PICKING='0')
							LEFT JOIN ESTADO_MERCADERIA_RL EM		(NOLOCK)	ON (RL.CLIENTE_ID=EM.CLIENTE_ID AND RL.EST_MERC_ID=EM.EST_MERC_ID) 	
					...
					----------------------------------------------------------------------------------------------------------------------------------
					--CON ESTE SP OBTENGO TODO LO ASIGNABLE DESDE BEST FIT.
					----------------------------------------------------------------------------------------------------------------------------------											
					EXEC DBO.RPT_PREPICKING_MULTIPLE_BEST_FIT	@CLIENTE_ID,@PRODUCTO_ID,@CANT_SOL,@NRO_LOTE,@NRO_PARTIDA,@NRO_SERIE,
																@EST_MERC_ID,@CAT_LOG_ID,@QTY_BEST_FIT_A	OUTPUT
																
				END
				ELSE
				BEGIN
					SET @QTY_BEST_FIT=0;
					SET @QTY_BEST_FIT_A=0;
				END
				
				SET @STOCK_DISP = (@STOCK_DISP   - @RESTA)
				
				SET @SALDO=	(@STOCK_DISP + @QTY_BEST_FIT_A) - @CANT_SOL;
				...
				WHERE	D.CLIENTE_ID = @CLIENTE_ID AND D.CODIGO_VIAJE=@CODIGO_VIAJE AND D.DOC_EXT = @DOC_EXT AND DD.NRO_LINEA = @NRO_LINEA
				GROUP BY
