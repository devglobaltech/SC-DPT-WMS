/****** Object:  StoredProcedure [dbo].[MOB_EMPAQUETADO_VERIFICA_CONTENEDORA]    Script Date: 09/07/2015 13:12:43 ******/
...
BEGIN
	...
	DECLARE @T_EMP	VARCHAR(1);
	
	SELECT	DISTINCT @ESTADO=PICKING.FIN_PICKING
	FROM	PICKING
	WHERE	((ISNUMERIC(@CONTENEDOR)='0') OR (PICKING.PALLET_PICKING =@CONTENEDOR))
			AND PICKING.NRO_UCEMPAQUETADO IS NULL
	...
	SELECT	DISTINCT @T_EMP=CP.TIPO_EMPAQUETADO_ID 
	FROM	PICKING P INNER JOIN CLIENTE_PARAMETROS CP ON(P.CLIENTE_ID=CP.CLIENTE_ID)
	WHERE	((ISNUMERIC(@CONTENEDOR)='0') OR (PALLET_PICKING =@CONTENEDOR))

	IF @T_EMP<>'4' begin
		RAISERROR('La configuracion del cliente no permite empaquetar por Hand Held.',16,1)
		return 
	END
	...
	Where	p.fin_picking in ('2')
			...
			and ((ISNUMERIC(@CONTENEDOR)='0') OR (P.PALLET_PICKING =@CONTENEDOR))
