/****** Object:  StoredProcedure [dbo].[MOB_CONSULTASTOCK2]    Script Date: 01/18/2016 17:58:40 ******/
...
BEGIN
	...
	group by X.ClienteID, X.ProductoID,X.DESCRIPCION,x.Unidad_id
			 ,X.Storage					,X.NaveID					,X.CalleCod 
			 ,X.CalleID					,X.ColumnaCod				,X.ColumnaID 
			 ,X.NivelCod 				,X.NivelID 					,X.EST_MERC_ID 
			 ,X.CategLogID 				,X.Nro_Serie				,X.Nro_Bulto 
			 ,X.Nro_Lote				,X.Nro_Despacho				,X.Nro_Partida 
			 ,X.prop1					,X.prop2					,X.prop3 
			 ,X.Fecha_Vencimiento		,X.Peso						,X.Unidad_Peso 
			 ,X.Volumen					,X.Unidad_Volumen			,X.Kit 
			 ,X.TIE_IN  ,X.TIE_IN_PADRE ,X.NRO_TIE_IN				,C.RAZON_SOCIAL 
			 ,PR.DESCRIPCION			,X.unidad_id				,X.moneda_id 
			 ,x.costo					,x.posicion_cod
END
ELSE
	BEGIN
	IF  @TipoOperacion=2
...
WHERE C.CLIENTE_ID = X.CLIENTEID 
     AND PR.CLIENTE_ID = X.CLIENTEID 
     AND PR.PRODUCTO_ID = X.PRODUCTOID 
     group by c.cliente_id,X.ClienteID, X.ProductoID
     ,X.Storage 
     ,X.NaveID 
     ,X.CalleCod 
     ,X.CalleID 
