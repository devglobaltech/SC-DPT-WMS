IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[MOB_TRANSFERENCIA_PREPICKING_PALLET]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[MOB_TRANSFERENCIA_PREPICKING_PALLET]
GO

CREATE PROCEDURE [dbo].[MOB_TRANSFERENCIA_PREPICKING_PALLET]
...
BEGIN
	...
	DECLARE CUR_RL_TR CURSOR FOR
		...
		FETCH NEXT FROM CUR_RL_TR INTO @VRLID,@CLIENTE_ID, @CATLOGID
				...
				BEGIN
					INSERT INTO RL_DET_DOC_TRANS_POSICION
							   (DOC_TRANS_ID,																	--1
								...
								DISPONIBLE,																		--12
								DOC_TRANS_ID_TR,																--13
								NRO_LINEA_TRANS_TR,																--14
								CLIENTE_ID,																		--15
								CAT_LOG_ID,																		--16
								EST_MERC_ID)																	--17

								(SELECT   DOC_TRANS_ID															--1
										, NRO_LINEA_TRANS														--2
										, POSICION_ACTUAL														--3
										, @NEWPOS																--4
										, CANTIDAD																--5
