...
BEGIN
	...
	BEGIN
		...
		-----------------------------------------------------------
		-- ESTE SEGMENTO HACE QUE SI EL MATERIAL ESTA EN LA ZONA
		-- DE PRE-PICKING NO HAGA FALTA TRANSFERIRLO.
		-----------------------------------------------------------
		UPDATE	PICKING	
		SET		TRANSF_TERMINADA='1'
		FROM	PICKING P INNER JOIN NAVE N
				ON(P.NAVE_COD=N.NAVE_COD)
		WHERE	ISNULL(P.FLG_PALLET_HOMBRE,'0')		= '1'
				AND P.CLIENTE_ID					= @CLIENTE_ID
				AND P.VIAJE_ID						= @VIAJE_ID
				AND ISNULL(N.ZONA_PRE_PICKING,'0')	= '1'
		-----------------------------------------------------------
