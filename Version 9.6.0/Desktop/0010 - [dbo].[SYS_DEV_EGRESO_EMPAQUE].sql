/****** Object:  StoredProcedure [dbo].[SYS_DEV_EGRESO_EMPAQUE]    Script Date: 11/16/2015 14:58:05 ******/
...
CREATE PROCEDURE [dbo].[SYS_DEV_EGRESO_EMPAQUE]
 @pclienteid	As varchar(15)	output,
...
BEGIN
	begin try
		...
		IF EXISTS (SELECT 1 FROM SYS_DEV_DOCUMENTO WHERE CLIENTE_ID=@pclienteid AND CODIGO_VIAJE = @pviaje)
			RETURN
		--Controlo que el viaje no este cerrado
		select @Qty=count(picking_id) from picking where cliente_id=@pclienteid and viaje_id=@pViaje and facturado='1'
		if (@Qty>0) begin
		...
		set @Qty=0
		select @Qty=count(picking_id) from picking where CLIENTE_ID=@pclienteid and (fin_picking in ('0','1') or fin_picking is null) and viaje_id=@pViaje
		if (@Qty>0) begin
		...
		where	dd.CLIENTE_ID=@pclienteid and dd.estado_gt is null and d.codigo_viaje=@pViaje
