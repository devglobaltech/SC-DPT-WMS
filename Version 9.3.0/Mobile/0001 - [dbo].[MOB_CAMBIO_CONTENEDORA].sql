...
/****** Object:  StoredProcedure [dbo].[MOB_CAMBIO_CONTENEDORA]    Script Date: 10/01/2014 15:03:02 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[MOB_CAMBIO_CONTENEDORA]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[MOB_CAMBIO_CONTENEDORA]
GO
	...
	BEGIN TRY
		...
		BEGIN
			...
			BEGIN
				SET @CURPICK=CURSOR FOR
					...
					ORDER BY P.CANTIDAD, P.FECHA_INICIO DESC;			
			END
		END
		OPEN @CURPICK
		...
		BEGIN
			UPDATE	PICKING SET USUARIO=@USR, PALLET_PICKING=@PALLET_PICKING,FECHA_INICIO=@FECHA_INICIO,
					FECHA_FIN = GETDATE(), CANT_CONFIRMADA = P.CANTIDAD
			FROM	PICKING P INNER JOIN DET_DOCUMENTO DD
					ON(P.DOCUMENTO_ID=DD.DOCUMENTO_ID AND P.NRO_LINEA=DD.NRO_LINEA)
			WHERE	P.VIAJE_ID=@VIAJE_ID
					AND P.PRODUCTO_ID=@PRODUCTO_ID
		...
		IF(@T_TOM=@T_NTOM)AND(@CANTIDAD>@SUM_PIK)
		BEGIN																
			UPDATE	PICKING SET USUARIO=@USR, PALLET_PICKING=@PALLET_PICKING,
