...
/****** Object:  UserDefinedFunction [dbo].[FX_SERIALIZADO_BEST_FIT]    Script Date: 08/21/2014 15:02:54 ******/
...
BEGIN
	...
	SELECT	@PC_ACTIVO=CP.flg_pallet_completo
	FROM	PICKING P INNER JOIN CLIENTE_PARAMETROS CP
			ON(P.CLIENTE_ID=CP.CLIENTE_ID)
	WHERE	PROP1=@PALLET

	Set @Retorno=5
	
	IF @PC_ACTIVO='0' BEGIN
		RETURN @RETORNO
	END
	--------------------------------------------------------------------------------------------------------------------
