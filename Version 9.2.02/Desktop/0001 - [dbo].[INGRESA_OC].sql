/****** Object:  StoredProcedure [dbo].[INGRESA_OC]    Script Date: 07/10/2014 12:34:49 ******/
  ...
  IF @NRO_REG_CONTENEDORAS>0
    BEGIN
     ...
     BEGIN
      ...
      VALUES(@doc_id, @Nrolinea , @cliente_id , @producto_id , @cantidad , NULL   , @cat_log_id  , '0'  , NULL   , @VPARTIDA , @unidad_id , @descripcion , '1'    , '1'  ,@cantidad   , @nro_pallet ,@dext , @NROBULTO  , @NRO_LOTE)
  ...
  ELSE
   BEGIN
    ...
    if @cntdd<>@CntIng begin 
      ...
      INSERT INTO det_documento (documento_id, nro_linea , cliente_id , producto_id , cantidad , cat_log_id , cat_log_id_final , tie_in , fecha_vencimiento , nro_partida , unidad_id  , descripcion , busc_individual , item_ok , cant_solicitada , prop1 , prop2, nro_bulto ,nro_lote)
      VALUES(@doc_id, @Nrolinea , @cliente_id , @producto_id , @cantidad , NULL   , @cat_log_id  , '0'  , NULL   , @VPARTIDA , @unidad_id , @descripcion , '1'    , '1'  ,@qtyBO   , @nro_pallet ,@DOC_EXT , NULL  , @NRO_LOTE)
    end 
   END
  --Documento a Ingreso.
  ...
  WHERE (CLIENTE_ID = @CLIENTE_ID) AND (ORDEN_COMPRA = @oc) AND (PRODUCTO_ID = @producto_id)
    ...
    AND (((@NRO_PARTIDA = '' OR @NRO_PARTIDA IS NULL) AND (NRO_PARTIDA = '' OR NRO_PARTIDA IS NULL)) OR @NRO_PARTIDA = NRO_PARTIDA)
     ...
     BEGIN
	  ...
	  Select	NULL, NULL, NULL, NULL, dd.cantidad, NULL, NULL, NULL, NULL, @PREING, dd.documento_id, dd.nro_linea, NULL, NULL, NULL, NULL, NULL, dd.cliente_id, @catlogid,@CAT_LOG_ID,NULL
	  from		det_documento dd
	  where		dd.documento_id=@doc_id
