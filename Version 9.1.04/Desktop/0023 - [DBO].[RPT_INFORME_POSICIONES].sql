...
/****** Object:  StoredProcedure [dbo].[RPT_INFORME_POSICIONES]    Script Date: 02/10/2014 11:59:24 ******/
...
FROM	(
	...
	SELECT	Q2.NAVE AS NAVE_COD,
			...
			ISNULL (SUM (Q2.VOL_01),0) AS VOLUMEN_POSICIONES_OCUPADAS
	FROM	
		(SELECT Q1.NAVE, Q1.POSICION_ACTUAL, SUM(VOL_01) AS VOL_01
		FROM
			(SELECT	N.NAVE_COD AS NAVE, RL.POSICION_ACTUAL, RL.CANTIDAD * (ISNULL(P.LARGO,0) * ISNULL(P.ALTO,0) * ISNULL(P.ANCHO,0))/1000000 AS VOL_01
			FROM RL_DET_DOC_TRANS_POSICION RL
				INNER JOIN DET_DOCUMENTO_TRANSACCION DDT ON (RL.DOC_TRANS_ID = DDT.DOC_TRANS_ID AND RL.NRO_LINEA_TRANS = DDT.NRO_LINEA_TRANS)
				INNER JOIN DET_DOCUMENTO DD ON (DDT.DOCUMENTO_ID = DD.DOCUMENTO_ID AND DDT.NRO_LINEA_DOC = DD.NRO_LINEA)
				INNER JOIN DOCUMENTO D ON (DD.DOCUMENTO_ID = D.DOCUMENTO_ID)
		...
		GROUP BY Q1.NAVE,Q1.POSICION_ACTUAL			
	) Q2			
	GROUP BY Q2.NAVE
	...
	SELECT	Q2.NAVE AS NAVE,
			0 AS POSICIONES_LIBRES,
			0 AS VOLUMEN_POSICIONES_LIBRES,
			ISNULL(SUM (0),0) AS POSICIONES_OCUPADAS,
			ISNULL (SUM (VOL_01),0) AS VOLUMEN_POSICIONES_OCUPADAS
	FROM	(
		SELECT	N.NAVE_COD AS NAVE, 
				RL.NAVE_ACTUAL, 				
				RL.CANTIDAD * (ISNULL(P.LARGO,0) * ISNULL(P.ALTO,0) * ISNULL(P.ANCHO,0))/1000000 AS VOL_01		
		FROM	RL_DET_DOC_TRANS_POSICION RL		
				INNER JOIN DET_DOCUMENTO_TRANSACCION DDT ON (RL.DOC_TRANS_ID = DDT.DOC_TRANS_ID AND RL.NRO_LINEA_TRANS = DDT.NRO_LINEA_TRANS)
				INNER JOIN DET_DOCUMENTO DD ON (DDT.DOCUMENTO_ID = DD.DOCUMENTO_ID AND DDT.NRO_LINEA_DOC = DD.NRO_LINEA)
				INNER JOIN DOCUMENTO D ON (DD.DOCUMENTO_ID = D.DOCUMENTO_ID)
		...
		WHERE	N.NAVE_TIENE_LAYOUT = 0 
				AND RL.NAVE_ACTUAL IS NOT NULL 
				AND N.NAVE_ID NOT IN ('1','2')
