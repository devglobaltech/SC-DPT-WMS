...
/****** Object:  StoredProcedure [dbo].[SYS_DEV_EGRESO]    Script Date: 12/18/2013 10:28:13 ******/
...
BEGIN
	begin try
		...
		set @cur =cursor for
		select distinct cliente_id from picking(nolock) where viaje_id=@pviaje
		...
		while @@FETCH_STATUS=0 begin
			...
			where	cliente_id=(select distinct cliente_id from picking(nolock) where viaje_id=@pviaje and CLIENTE_ID=@cli)
			...
			where	viaje_id=ltrim(rtrim(upper(@pViaje))) and st_control_exp='0' and P.CLIENTE_ID=@cli	
				
			if @controla='1'
			begin
				Select @Qty=count(isnull(estado,'0')) from picking where viaje_id=ltrim(rtrim(upper(@pViaje)))and ISNULL(ESTADO,'0')<>'2' and CLIENTE_ID=@cli
			...
			where	((viaje_id=ltrim(rtrim(upper(@pViaje))))or(d.nro_remito=@pviaje))
					and st_control_exp='0'
					and P.CLIENTE_ID=@cli
					
			if @controla='1'
			begin
				Select	@Qty=count(p.st_control_exp) 
				from	picking p inner join DOCUMENTO d
						on(d.DOCUMENTO_ID=p.DOCUMENTO_ID)
				where	((p.viaje_id=ltrim(rtrim(upper(@pViaje))))or(d.NRO_REMITO=@pviaje))
						and p.st_control_exp='0' 
						and p.CLIENTE_ID=@cli
...
print('SDD')
			...
			---METO LAS CABECERAS QUE NO INGRESARON POR FALTA DE STOCK
			insert into sys_dev_documento
			select	distinct 
					sid.CLIENTE_ID, 
					CASE WHEN sid.tipo_documento_id='E04' THEN 'E05' WHEN sid.tipo_documento_id='E08' THEN 'E09' ELSE sid.tipo_documento_id END, 
					...
					sid.ORDEN_DE_COMPRA, 
					sid.OBSERVACIONES, 
					cast(SID.cpte_prefijo as varchar(20)) + cast(SID.cpte_numero  as varchar(20)), 
					sid.NRO_DESPACHO_IMPORTACION, 
					sid.DOC_EXT, 
					sid.CODIGO_VIAJE, 
					sid.INFO_ADICIONAL_1, 
