...
/****** Object:  StoredProcedure [dbo].[CAMB_CATLOG]    Script Date: 03/13/2014 10:46:45 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[CAMB_CATLOG]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[CAMB_CATLOG]
GO
...
CREATE PROCEDURE [dbo].[CAMB_CATLOG]
...
BEGIN
	...
	--VERIFICA LA CANTIDAD DISPONIBLE PARA AJUSTAR
	...
	BEGIN
		...
		raiserror('ERROR: NO ES POSIBLE MODIFICAR LA CATEGORIA LOGICA A %s UNIDADES MAXIMO DISPONIBLE A MODIFICAR : %s',16,1,@STR_TOTAL_A_CAMBIAR,@STR_MAX_A_DESC)
		return
	END
		...
		Begin	
			...
			IF (@CANT_TOTAL_UBIC <= @TOTAL_A_CAMBIAR)
				BEGIN
					...
					UPDATE RL_DET_DOC_TRANS_POSICION SET CAT_LOG_ID = @NEW_CAT_LOG_ID
					WHERE RL_ID = @RLID	
				END
			ELSE
				BEGIN
				...
				BEGIN
					CLOSE @RsRlId
					DEALLOCATE @RsRlId
					EXEC GET_VALUE_FOR_SEQUENCE 'AJUSTE_CAT_LOG',@SECUENCIA OUTPUT
					EXEC Ajuste_Categoria_Logica @CLIENTE_ID,'ST01',@SECUENCIA,@PRODUCTO_ID,@NEW_CANT,@EST_MERC_ID,@NEW_CAT_LOG_ID,@NRO_BULTO,'',@NRO_LOTE,@PROP1,@FECHA_VTO,@NRO_DESPACHO,@NRO_PARTIDA,@UNIDAD_ID,NULL,@NAVE_COD,@CAT_LOG_ID
					EXEC SYS_AUDITORIA_CAT_MERC_INSERT_RECORD 'CATEGORIA LOGICA',@RLID,@CLIENTE_ID,@NAVE_ID,@POSICION_ID,@CAT_LOG_ID,@NEW_CAT_LOG_ID,@CANTIDAD,@NEW_CANT
					EXEC AUDITORIA_HIST_INSERT_CATLOG @RLID,@CAT_LOG_ID,@NEW_CAT_LOG_ID,@NEW_CANT
					RETURN
				END
				Fetch Next From @RsRlId into
											@RLID,
...
BEGIN
	EXEC GET_VALUE_FOR_SEQUENCE 'AJUSTE_CAT_LOG',@SECUENCIA OUTPUT
	EXEC Ajuste_Categoria_Logica @CLIENTE_ID,'ST01',@SECUENCIA,@PRODUCTO_ID,@TOTAL_A_CAMBIAR,@EST_MERC_ID,@NEW_CAT_LOG_ID,'',@NRO_LOTE,@PROP1,@FECHA_VTO,@NRO_DESPACHO,@NRO_PARTIDA,@UNIDAD_ID,NULL,@NAVE_COD,@CAT_LOG_ID
	EXEC SYS_AUDITORIA_CAT_MERC_INSERT_RECORD 'CATEGORIA LOGICA',@RLID,@CLIENTE_ID,@NAVE_ID,@POSICION_ID,@CAT_LOG_ID,@NEW_CAT_LOG_ID,@CANTIDAD,@TOTAL_A_CAMBIAR
	EXEC AUDITORIA_HIST_INSERT_CATLOG @RLID,@CAT_LOG_ID,@NEW_CAT_LOG_ID,@TOTAL_A_CAMBIAR
END*/
END
