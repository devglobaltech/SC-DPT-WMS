/****** Object:  StoredProcedure [dbo].[TAREAS_PICKING_D]    Script Date: 06/28/2013 17:06:19 ******/
...
AS
	...
	ELSE
		BEGIN
			IF @TQUERY='2'
				BEGIN
					--raiserror('ME QUIERO CORTAR LAS PELOTAS',16,1);
							...
							WHERE 		SP.FECHA_INICIO IS NULL
										...
										AND SP.SALTO_PICKING = (	SELECT 	MIN(ISNULL(SALTO_PICKING,0))
																	...
																	WHERE 	LTRIM(RTRIM(UPPER(VIAJE_ID)))=LTRIM(RTRIM(UPPER(@VIAJE_ID)))
																			...
																			AND FECHA_FIN IS NULL
																			AND CANT_CONFIRMADA IS NULL
																			AND ((@TOMARUTA='0') OR(LTRIM(RTRIM(UPPER(RUTA)))=LTRIM(RTRIM(UPPER(@RUTA_I))))))
