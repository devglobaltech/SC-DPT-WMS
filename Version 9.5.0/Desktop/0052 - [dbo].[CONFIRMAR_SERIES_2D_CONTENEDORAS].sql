/****** Object:  StoredProcedure [dbo].[CONFIRMAR_SERIES_2D_CONTENEDORAS]    Script Date: 04/27/2015 11:22:16 ******/
...
BEGIN
	...
	DECLARE @CANT_SERIES_CONTENEDORA	NUMERIC(20,0),
			...
			@MSG						VARCHAR(100),
			@SERIE_EGR					NUMERIC(20,0),
			@SERIE_PICK					NUMERIC(20,0)
	...
	SELECT	@SERIE_PICK=COUNT(P.NRO_SERIE)
	FROM	PICKING P INNER JOIN DET_DOCUMENTO DD
			ON(P.DOCUMENTO_ID=DD.DOCUMENTO_ID AND P.NRO_LINEA=DD.NRO_LINEA)
	WHERE	P.CLIENTE_ID=@CLIENTE_ID
			AND P.VIAJE_ID=@VIAJE_ID
	...
	SET @CANT_SERIES_CONTENEDORA=ISNULL(@CANT_SERIES_CONTENEDORA,0)-ISNULL(@SERIE_PICK,0)

	IF @CANT_SERIES>@CANT_SERIES_CONTENEDORA BEGIN
		RAISERROR('La cantidad de series que tomo supera a la cantidad de series disponibles en la contenedora. ',16,1)
		return
	END
	
	SET @CANT_SERIES_CONTENEDORA=@CANT_SERIES;
	...
	BEGIN
		...
		BEGIN
			...
			IF @USA_CERO='1' BEGIN
				...
				WHERE	P.CLIENTE_ID=@CLIENTE_ID
						AND P.PRODUCTO_ID=@PRODUCTO_ID
						AND P.NRO_SERIE=replicate('0', 9 - len(@SERIE_CONFORMADA)) + cast (@SERIE_CONFORMADA as varchar)
						AND D.STATUS <>'D40';
			END
			ELSE
			BEGIN
			...
			BEGIN
				...
				END ELSE BEGIN
					SET @MSG=replicate('0', 9 - len(@SERIE_CONFORMADA)) + cast (@SERIE_CONFORMADA as varchar)
				END
			...
			IF @USA_CERO='1' BEGIN						
				...
				SET		FECHA_INICIO=@F_INICIO, 
						...
						NRO_SERIE=replicate('0', 9 - len(@SERIE_CONFORMADA)) + cast (@SERIE_CONFORMADA as varchar),
		...
		BEGIN
			...
			SET		FECHA_INICIO=NULL,
					USUARIO=NULL
			WHERE	PICKING_ID IN(	SELECT	PICKING_ID
									FROM	PICKING P INNER JOIN DET_DOCUMENTO DD
											ON(P.DOCUMENTO_ID=DD.DOCUMENTO_ID AND P.NRO_LINEA=DD.NRO_LINEA)
									WHERE	P.CLIENTE_ID=@CLIENTE_ID
											AND P.VIAJE_ID=@VIAJE_ID
