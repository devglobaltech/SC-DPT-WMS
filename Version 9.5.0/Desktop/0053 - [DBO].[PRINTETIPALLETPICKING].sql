/****** Object:  StoredProcedure [dbo].[PrintEtiPalletPicking]    Script Date: 05/08/2015 12:35:28 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[PrintEtiPalletPicking]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[PrintEtiPalletPicking]
GO

CREATE PROCEDURE [dbo].[PrintEtiPalletPicking]
...
Begin
	...
	FROM	  PICKING P(NOLOCK) 
			  ...
			  INNER JOIN SUCURSAL S ON (D.SUCURSAL_DESTINO = S.SUCURSAL_ID AND D.CLIENTE_ID = S.CLIENTE_ID)
