/****** Object:  StoredProcedure [dbo].[ABAST_MOB_TAREAS]    Script Date: 06/08/2015 11:39:01 ******/
...
BEGIN	
	...
	IF @CONTROL=0 BEGIN
		...
		FROM    RL_PRODUCTO_POSICION_PERMITIDA R (NOLOCK)INNER JOIN POSICION P(NOLOCK)
				...
				LEFT JOIN (	SELECT   dd.cliente_id
                            ...
                            FROM    rl_det_doc_trans_posicion rl(NOLOCK)
                                    ...
                                    inner join categoria_logica cl  (NOLOCK)            on(rl.cliente_id=cl.cliente_id and rl.cat_log_id=cl.cat_log_id )
                                    inner join posicion p           (NOLOCK)            on(rl.posicion_actual=p.posicion_id)
				...
				INNER JOIN	RL_USUARIO_NAVE RL (NOLOCK)
				ON(P.NAVE_ID =RL.NAVE_ID AND RL.USUARIO_ID=@USUARIO)	
				INNER JOIN RL_SYS_CLIENTE_USUARIO CU (NOLOCK)
				ON(R.CLIENTE_ID=CU.CLIENTE_ID AND CU.USUARIO_ID=@USUARIO)
				INNER JOIN (
							SELECT	X.CLIENTE_ID,X.PRODUCTO_ID,SUM(X.CANTIDAD) QTY
							...
							GROUP BY X.CLIENTE_ID,X.PRODUCTO_ID
				)STK ON(R.CLIENTE_ID=STK.CLIENTE_ID AND R.PRODUCTO_ID =STK.PRODUCTO_ID)				
