/****** Object:  StoredProcedure [dbo].[LocatorEgreso_pallet_completo]    Script Date: 10/17/2014 12:19:42 ******/
...
Begin 
	...
	BEGIN TRY
		...
		BEGIN
			...
			BEGIN
						...
						WHERE	rl.doc_trans_id_egr is null
								...
								and exists (select  1 
											...
											where	ddegr.documento_id = @DOCIDPIVOT 
													...
													and ddegr.cliente_id=dd.cliente_id
				...
				Begin	
					...
					--Set @miPrint='PRODUCTO: '+@Producto_id + '| Remanente: ' + CAST(@vQtyResto as varchar)
					--print(@miPrint)
					...
					if @vQtyResto>=@QTYPallet begin
						set @AsignaPallet='1'
					end
										
					if (@vQtyResto>0) and(@AsignaPallet='1')
					begin   
						...
						begin
							...
							SELECT @Asignado=SUM(CANTIDAD) FROM det_documento_aux WHERE DOCUMENTO_ID=@pDocumento_id and PRODUCTO_ID=@Producto_id AND TRACE_BACK_ORDER = @NROLINEAPIVOT
							
							if @asignado is null begin set @Asignado=0 end
							
							set @vQtyResto=@QtySol-@Asignado
							
							set @vQtyResto=@vQtyResto-@QTYPallet
							...
							SELECT	@pDocumento_id,RL.RL_ID,@Cliente_id, @Producto_id,rl.cantidad, rl.rl_id, 0,'1',getdate(),'N'
							FROM	DET_DOCUMENTO DD INNER JOIN DET_DOCUMENTO_TRANSACCION DDT
									ON(DD.DOCUMENTO_ID=DDT.DOCUMENTO_ID AND DD.NRO_LINEA=DDT.NRO_LINEA_DOC)
									INNER JOIN RL_DET_DOC_TRANS_POSICION RL
							...
							insert into det_documento_aux (	documento_id,nro_linea,cliente_id,producto_id,cantidad,est_merc_id,
															...
															unidad_id,tie_in,item_ok,cat_log_id_final,prop1,prop2,prop3,cant_solicitada,nro_serie,TRACE_BACK_ORDER)
							...
							SELECT	@PDOCUMENTO_ID,RL.RL_ID,@CLIENTE_ID, @PRODUCTO_ID,RL.CANTIDAD,RL.EST_MERC_ID,RL.CAT_LOG_ID,DD.NRO_BULTO,DD.DESCRIPCION,DD.NRO_LOTE,DD.FECHA_VENCIMIENTO,DD.NRO_DESPACHO,
									DD.NRO_PARTIDA,DD.UNIDAD_ID,DD.TIE_IN,DD.ITEM_OK,DD.CAT_LOG_ID_FINAL,DD.PROP1,DD.PROP2,DD.PROP3,@QTYSOL,DD.NRO_SERIE,@NROLINEAPIVOT
							FROM	DET_DOCUMENTO DD INNER JOIN DET_DOCUMENTO_TRANSACCION DDT
									ON(DD.DOCUMENTO_ID=DDT.DOCUMENTO_ID AND DD.NRO_LINEA=DDT.NRO_LINEA_DOC)
									INNER JOIN RL_DET_DOC_TRANS_POSICION RL
									ON(RL.DOC_TRANS_ID=DDT.DOC_TRANS_ID AND RL.NRO_LINEA_TRANS=DDT.NRO_LINEA_TRANS)
							...
							SELECT @vNroLinea=MAX(NRO_LINEA) FROM DET_DOCUMENTO_AUX WHERE DOCUMENTO_ID=	@PDOCUMENTO_ID																					
							...
							begin
								...
								begin 
									...
									insert into det_documento_aux (
												...
												,'1',@CAT_LOG_ID,@PROP1,@PROP2,@PROP3,@QtySol,@nro_serie,@NROLINEAPIVOT)
									insert into #tmp_consumo_locator_egr values (@vRl_id, @Cantidad)	
								end
								else 
								begin
		...
		insert into det_documento select 	DOCUMENTO_ID,	NRO_LINEA,	CLIENTE_ID,
		...
		*/
		update	DET_DOCUMENTO 
		set		NRO_SERIE=a.nro_serie
		from	DET_DOCUMENTO dd inner join DET_DOCUMENTO_AUX a
				on(dd.DOCUMENTO_ID=a.DOCUMENTO_ID and dd.NRO_LINEA=a.NRO_LINEA)
