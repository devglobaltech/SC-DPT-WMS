/****** Object:  View [dbo].[view_web_ingreso_detallado]    Script Date: 12/10/2015 18:01:27 ******/
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[view_web_ingreso_detallado]'))
DROP VIEW [dbo].[view_web_ingreso_detallado]
GO
...
SELECT	d.CLIENTE_ID						as cliente_id,
		...
		AHG.fecha_auditoria					as fecha_guardado,
...
FROM	DOCUMENTO D inner join DET_DOCUMENTO dd
		...
		left join (		select	DISTINCT 
								DOCUMENTO_ID, NRO_LINEA_DOC,USUARIO_ID, FECHA_AUDITORIA
						from	AUDITORIA_HISTORICOS a
...
group by
		...
		SU2.NOMBRE,			AHG.FECHA_AUDITORIA 
