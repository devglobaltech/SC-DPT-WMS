...
/****** Object:  StoredProcedure [dbo].[PRINT_ETI_BULTOEGR]    Script Date: 01/28/2015 15:07:56 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[PRINT_ETI_BULTOEGR]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[PRINT_ETI_BULTOEGR]
GO

CREATE PROCEDURE [dbo].[PRINT_ETI_BULTOEGR]
...
*/
	...
	WHERE 	SPV.PRIORIDAD = ( SELECT 	MIN(PRIORIDAD) FROM	PRIORIDAD_VIAJE	WHERE	LTRIM(RTRIM(UPPER(VIAJE_ID)))=LTRIM(RTRIM(UPPER(SP.VIAJE_ID))))								
			...
			AND SP.POSICION_COD IN(	SELECT 	POSICION_COD
									...
									WHERE 	VEHICULO_ID=@VH
											AND CN.CALLE_ID IN(SELECT CALLE_ID FROM #TEMP_PARAM_ETI WHERE NAVE_ID IS NULL and calle_id is not null)
									UNION 
									SELECT 	NAVE_COD AS POSICION_COD
									FROM	RL_VEHICULO_POSICION V INNER JOIN NAVE N2
											ON(V.NAVE_ID=N2.NAVE_ID)
									WHERE	VEHICULO_ID=@VH
											AND N2.NAVE_ID IN(SELECT nave_id FROM #TEMP_PARAM_ETI WHERE calle_id IS NULL and nave_id is not null)
									)
			AND SP.VIAJE_ID=@VIAJE_ID
	GROUP BY	
			SP.VIAJE_ID, SP.PRODUCTO_ID,SP.DESCRIPCION, SP.RUTA,SP.POSICION_COD,SP.TIPO_CAJA,SP.PROP1,PROD.UNIDAD_ID,SPV.PRIORIDAD,POS.ORDEN_PICKING
			, SP.CLIENTE_ID, PROD.VAL_COD_EGR, SP.HIJO,CP.FLG_SOLICITA_LOTE, DD.PROP2
	ORDER BY	
			SPV.PRIORIDAD ASC,SP.TIPO_CAJA DESC, POS.ORDEN_PICKING, SP.POSICION_COD ASC, SP.PRODUCTO_ID
	...
	Begin
		...
		Set @CurDoc=Cursor For
			...
			Where	Viaje_id=@VIAJE_ID and p.producto_id=@Prod 
					and posicion_cod=@Pos 
		...
		While @@Fetch_Status=0
		Begin			
			Select	@SucursalId=D.Sucursal_Destino, @SucCod=s.nombre,@SUC_CALLE = S.CALLE, @Picking_id=P.Picking_Id,@CAT_LOG_ID_FINAL = DD.CAT_LOG_ID_FINAL,
			...
			Begin
				...
				If @Cont=1
				Begin
					SELECT  @MYPOS=IsNull(CharIndex(Cast(@Cont as varchar)+'|',BULTOS_NO_CONTROLADOS,1),0)
					FROM	PICKING
					WHERE	DOCUMENTO_ID=@Documento AND NRO_LINEA=@nrolinea
					
					IF @MYPOS=0
					BEGIN
						UPDATE PICKING SET BULTOS_NO_CONTROLADOS=ISNULL(BULTOS_NO_CONTROLADOS,'')+Cast(@Cont as varchar)+'|' 
	...
	Deallocate @Cur
	commit transaction
	--Select t.Sucursal_ID, t.OS, t.Codigo_ID, t.descripcion, t.bulto, t.ID_Pick, t.Calle, t.Printer, T.PRODUCTO_ID From #NEW_ETI t  --order by id_eti desc
End --procedure
