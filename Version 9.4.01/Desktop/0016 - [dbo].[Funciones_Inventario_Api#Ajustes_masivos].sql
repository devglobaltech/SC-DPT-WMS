...
/****** Object:  StoredProcedure [dbo].[Funciones_Inventario_Api#Ajustes_masivos]    Script Date: 01/29/2015 10:47:32 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Funciones_Inventario_Api#Ajustes_masivos]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Funciones_Inventario_Api#Ajustes_masivos]
GO

CREATE        Procedure [dbo].[Funciones_Inventario_Api#Ajustes_masivos]
	...
	Begin
		...
		ELSE
			BEGIN
				...
				--PUEDE OCURRIR QUE SE TRATE DE UN PRODUCTO QUE ACTUALMENTE SE ENCUENTRE TOTALMENTE EN PRE-EGRESO
				
				SELECT 	@CHECK = ISNULL(SUM(DD.CANTIDAD - (ISNULL(P.CANT_CONFIRMADA,0))),0) 
				...
				WHERE D.STATUS = 'D30'                                                                     
						...
						AND DD.CLIENTE_ID = @V_CLIENTE_ID
						AND DD.PRODUCTO_ID = @V_PRODUCTO_ID
						AND (@V_NAVE_ID IS NULL OR RL.NAVE_ANTERIOR = @V_NAVE_ID)
						AND (@V_POSICION_ID IS NULL OR RL.POSICION_ANTERIOR = @V_POSICION_ID)    				
				...
				IF @CHECK <= 0 
					...
					END				
				ELSE
					--SI ES UN AJUSTE POSITIVO CREO UNA RL
					IF @V2_SIGNO = '-'
						BEGIN
							ROLLBACK TRAN 
							UPDATE DET_INVENTARIO_AJU SET PROCESADO = 'E' WHERE CURRENT OF @cur		
					...
					ELSE
						...
						INSERT INTO RL_DET_DOC_TRANS_POSICION
							...
							(SELECT TOP 1
								   ...
								   ,RL.NRO_LINEA
								   ,'1'
								   ,NULL --DOC_TRANS_ID_EGR
								   ,NULL --NRO_LINEA_TRANS_EGR
								   ,RL.DOC_TRANS_ID_TR
	...
	BEGIN
		...
		Begin	
			...
			Insert Into RL_DET_DOC_TRANS_POSICION (
						...
						EST_MERC_ID)
			Values (@P_Doc_Trans_Id, @P_Nro_Linea_Trans, NULL, @V_POSICION_ID, @P_Cantidad, NULL, NULL, NULL, null,@V_NAVE_ID, @P_Documento_Id, @P_Nro_Linea_Doc, @P_DISPONIBLE, null, null, null, null, @V_CLIENTE_ID, @P_Cat_Log_Id,@P_Cat_Log_Id_Final,null)
