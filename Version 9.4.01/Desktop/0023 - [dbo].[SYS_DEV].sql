...
/****** Object:  StoredProcedure [dbo].[SYS_DEV]    Script Date: 02/06/2015 16:04:47 ******/
...
BEGIN
	...
	IF (@TIPOPERACION = 'EGR')
		BEGIN
			...
			SELECT	@CLIENTE_ID=SID.CLIENTE_ID, @DOC_EXT=SID.DOC_EXT, @VIAJE_ID=SID.CODIGO_VIAJE
			FROM	SYS_INT_DET_DOCUMENTO SIDD INNER JOIN SYS_INT_DOCUMENTO SID
			ON (SIDD.DOC_EXT = SID.DOC_EXT AND SID.CLIENTE_ID = SIDD.CLIENTE_ID)
			WHERE	DOCUMENTO_ID = @DOCUMENTO_ID
			
			EXEC SYS_DEV_EGRESO @VIAJE_ID
		RETURN
	END	
