/****** Object:  StoredProcedure [dbo].[RPT_MOVIMIENTOS]    Script Date: 01/26/2015 09:25:59 ******/
...
BEGIN
	...
	*/
	...
	IF (@PRODUCTO_ID IS NOT NULL)AND((@COD_VIAJE IS NULL)AND(@COD_PEDIDO IS NULL))
	BEGIN
	...
	ELSE
	BEGIN
		--SACO EL CASO DONDE EL PRODUCTO NO ESTA PRESENTE PERO LE PUSIERON EL CODIGO DE VIAJE O PEDIDO.
		SELECT 	 DBO.GET_SALDO_INICIAL(AH.CLIENTE_ID,AH.PRODUCTO_ID,@P_FECHA_I,@F_HASTA)AS [SD_INICIAL]
				,AH.CLIENTE_ID + ' - ' +C.RAZON_SOCIAL			AS [CLIENTE_RZ]
				,AH.PRODUCTO_ID
				,PA.DESCRIPCION									AS [DESC_OP]
		...
		WHERE	((@NRO_PALLET IS NULL)OR(AH.PROP1=@NRO_PALLET))
				...
				AND ((@NRO_DESPACHO IS NULL) OR (AH.NRO_DESPACHO = @NRO_DESPACHO)) 
				AND EXISTS (		SELECT	1
									FROM	vPICKING P1 (NOLOCK) INNER JOIN vDET_DOCUMENTO DD1	(NOLOCK) ON(P1.DOCUMENTO_ID=DD1.DOCUMENTO_ID AND P1.NRO_LINEA=DD1.NRO_LINEA)
											INNER JOIN vDOCUMENTO D1 							(NOLOCK) ON(DD1.DOCUMENTO_ID=D1.DOCUMENTO_ID)
											INNER JOIN TIPO_COMPROBANTE TC1						(NOLOCK) ON(D1.TIPO_COMPROBANTE_ID=TC1.TIPO_COMPROBANTE_ID)
											INNER JOIN CLIENTE C1								(NOLOCK) ON(C1.CLIENTE_ID=P1.CLIENTE_ID)
											LEFT JOIN SYS_USUARIO SU1							(NOLOCK) ON(P1.USUARIO_CONTROL_FAC=SU1.USUARIO_ID)
											LEFT JOIN SUCURSAL	S1								(NOLOCK) ON(S1.CLIENTE_ID=D1.CLIENTE_ID AND S1.SUCURSAL_ID=D1.SUCURSAL_DESTINO)
											LEFT JOIN tmpSeriesPicking SP1						(NOLOCK) ON(SP1.PICKING_ID = P1.PICKING_ID)
									WHERE	1=1
											AND ((@NRO_PALLET IS NULL)OR(P1.PROP1=@NRO_PALLET))
											AND 16 In (Select Tipo_Auditoria_Id from #TEMP_CRITERIOS_RPT)
											AND((@CLIENTE_ID IS NULL) OR (P1.CLIENTE_ID=@CLIENTE_ID))
											AND((@PRODUCTO_ID IS NULL) OR (P1.PRODUCTO_ID=@PRODUCTO_ID))
											AND((@NRO_PARTIDA IS NULL) OR (DD1.NRO_PARTIDA=@NRO_PARTIDA))
											AND((@FECHA_VTO IS NULL) OR (DD1.FECHA_VENCIMIENTO=@FECHA_VTO))
											AND((@USUARIO IS NULL) OR (P1.USUARIO_CONTROL_FAC=@USUARIO))
											AND((@NRO_LOTE IS NULL) OR (DD1.NRO_LOTE=@NRO_LOTE))
											AND((@PROP2 IS NULL) OR (DD1.PROP2=@PROP2))
											AND((@PROP3 IS NULL) OR (DD1.PROP3=@PROP3))
											AND((@COD_PEDIDO IS NULL) OR(D1.NRO_REMITO LIKE  '%'+ @COD_PEDIDO +  '%'))
											AND((@COD_VIAJE IS NULL) OR(D1.NRO_DESPACHO_IMPORTACION LIKE '%' + @COD_VIAJE + '%'))
											AND ((@QSERIE IS NULL) OR (P1.NRO_SERIE = @QSERIE))						
											AND ((@NRO_DESPACHO IS NULL) OR (DD1.NRO_DESPACHO = @NRO_DESPACHO)) 
											AND (DD1.CLIENTE_ID=DD.CLIENTE_ID)
											AND (DD1.PRODUCTO_ID=DD.PRODUCTO_ID)
											AND ((DD1.NRO_LOTE IS NULL)OR(DD1.NRO_LOTE=DD.NRO_LOTE))
											AND ((DD1.NRO_PARTIDA IS NULL)OR(DD1.NRO_PARTIDA=DD.NRO_PARTIDA))
											AND ((P1.NRO_SERIE IS NULL)OR(P1.NRO_SERIE=DD.NRO_SERIE))
							)	
		UNION	
		--Egresos
		SELECT 
