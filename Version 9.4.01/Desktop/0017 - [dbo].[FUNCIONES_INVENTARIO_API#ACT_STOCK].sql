...
/****** Object:  StoredProcedure [dbo].[FUNCIONES_INVENTARIO_API#ACT_STOCK]    Script Date: 02/02/2015 12:21:44 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[FUNCIONES_INVENTARIO_API#ACT_STOCK]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[FUNCIONES_INVENTARIO_API#ACT_STOCK]
GO

CREATE PROCEDURE [dbo].[FUNCIONES_INVENTARIO_API#ACT_STOCK]
		...
		FROM    DET_INVENTARIO DI 
            INNER JOIN (  SELECT  XX.nave_id,                      
                          ...
                          FROM (  SELECT  DISTINCT
                                          ...
                                          case when isnull(pr.ingLoteProveedor,'0') ='1' OR isnull(pr.LOTE_AUTOMATICO,'0') ='1'  then det_doc.nro_lote     else '9999XX' end as nro_lote,
                                          case when isnull(pr.ingpartida,'0')='1' OR isnull(pr.NRO_PARTIDA_AUTOMATICO,'0')='1' then det_doc.nro_partida  else '9999XX' end as nro_partida
						                      FROM    documento doc                                          
                                          ...
                                          case when isnull(pr.ingLoteProveedor,'0') ='1' OR isnull(pr.LOTE_AUTOMATICO,'0') ='1' then det_doc.nro_lote     else '9999XX' end,
                                          case when isnull(pr.ingpartida,'0')='1'OR isnull(pr.NRO_PARTIDA_AUTOMATICO,'0') ='1' then det_doc.nro_partida  else '9999XX' end
                                  ...
                                  ------------------------------------------------------------------------------
                                          ...
                                          case when isnull(pr.ingLoteProveedor,'0') ='1' OR isnull(pr.LOTE_AUTOMATICO,'0') ='1'  then dd.nro_lote    else '9999XX' end as nro_lote,
                                          case when isnull(pr.ingpartida,'0')='1' OR isnull(pr.NRO_PARTIDA_AUTOMATICO,'0')='1'  then dd.nro_partida else '9999XX' end as nro_partida
                                          ...
                                          case when isnull(pr.ingLoteProveedor,'0') ='1' OR isnull(pr.LOTE_AUTOMATICO,'0') ='1' then dd.nro_lote     else '9999XX' end,
                                          case when isnull(pr.ingpartida,'0') ='1' OR  isnull(pr.NRO_PARTIDA_AUTOMATICO,'0')='1' then dd.nro_partida  else '9999XX' end
                                  ) XX
