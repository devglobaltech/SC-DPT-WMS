/****** Object:  StoredProcedure [dbo].[SYS_DEV_I01]    Script Date: 01/21/2015 15:48:52 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SYS_DEV_I01]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SYS_DEV_I01]
GO

CREATE     PROCEDURE [dbo].[SYS_DEV_I01]
...
begin 
	...
	select	sidd.DOC_EXT, 
			...
			sidd.PROP1, 
			dd.PROP2, --NRO_LOTE 
			Isnull(dd.PROP3,Dbo.Fx_GetNroRemitoDO(DD.DOCUMENTO_ID)), 
			sidd.LARGO, 
			sidd.ALTO, 
			sidd.ANCHO, 
			sidd.DOC_BACK_ORDER, 
	...
	group by
			...
			, DBO.GET_NAVE_ID(DD.DOCUMENTO_ID,DD.NRO_LINEA)
			, DBO.GET_NAVE_COD(DD.DOCUMENTO_ID,DD.NRO_LINEA)
			, sidd.CUSTOMS_1,sidd.CUSTOMS_2, sidd.CUSTOMS_3 		
end
END
