/****** Object:  StoredProcedure [dbo].[LOCATOR_ING_X_ALTURA]    Script Date: 20/04/2016 21:06:55 ******/
...
AS
  ...
  DECLARE @FLG_CONTROL_CANT_PALLET VARCHAR(1)
  DECLARE @MP				NUMERIC(20)
BEGIN

	SELECT	@MULTIPROD=COUNT(DISTINCT PRODUCTO_ID)
	FROM	DET_DOCUMENTO
	WHERE	PROP1=@NROPALLET

	SELECT	@MP=COUNT(DISTINCT PRODUCTO_ID)
	FROM	DET_DOCUMENTO
	WHERE	PROP1=@NROPALLET
	...
	SELECT	@FLG_VOL_PESO=ISNULL(C.FLG_ING_VOL_PESO,'0'), @FLG_CONTROL_CANT_PALLET=ISNULL(FLG_CONTROL_CANT_PALLET,'0')
	...
	BEGIN
		...
		IF @VCANT>0 BEGIN
			...
			WHERE 	1=1
					...
					and ((@FLG_CONTROL_CANT_PALLET='1')or( P.POS_VACIA='1')) --NO ES MAS VALIDO.
		...
		END ELSE BEGIN
			...
			FROM	(	SELECT	POSICION_ID, ORDEN_LOCATOR
						FROM	POSICION P
						WHERE	1=1 
								and ((@FLG_CONTROL_CANT_PALLET='1')or( P.POS_VACIA='1' AND NOT EXISTS (SELECT 1 FROM RL_DET_DOC_TRANS_POSICION R WHERE R.POSICION_ACTUAL=P.POSICION_ID)))
		...
		BEGIN
			...
			WHILE (@@FETCH_STATUS=0) OR (@CASO IS NOT NULL) BEGIN	
				...
				FROM(   SELECT   P.POSICION_ID					AS POSICION_ID
						...
						WHERE	((@FLG_CONTROL_CANT_PALLET='1')OR(P.POS_VACIA='1' ))
								AND P.POS_LOCKEADA='0'
								AND ((@FLG_CONTROL_CANT_PALLET='1')OR(P.POSICION_ID NOT IN(SELECT 	ISNULL(POSICION_ID,0)FROM SYS_LOCATOR_ING)))
								...
								AND((@FLG_CONTROL_CANT_PALLET='0' OR @MP>1)OR(DBO.GUARDADO_PALLET_INGRESO_CANT_PALLET(@CLIENTEID, @PRODUCTOID, @VPOS_EVAL)=1))
						...
						WHERE	N.DISP_INGRESO='1' AND N.PRE_INGRESO='0' 
								...
								AND((@FLG_CONTROL_CANT_PALLET='0')OR(DBO.GUARDADO_PALLET_INGRESO_CANT_PALLET_NAVE(@CLIENTEID, @PRODUCTOID, @VPOS_EVAL)=1))
				)AS X
				ORDER BY 
			...
			WHILE (@@FETCH_STATUS=0) OR (@CASO IS NOT NULL) BEGIN	
				...
				FROM(   SELECT 	 P.POSICION_ID  AS POSICION_ID
						...
						WHERE	((@FLG_CONTROL_CANT_PALLET='1')OR(P.POS_VACIA='1')) 
								...
								AND ((@FLG_VOL_PESO='0')OR(DBO.GUARDADO_PALLET_INGRESO_VOLUMEN(@DOCUMENTO_ID,@NRO_LINEA,@NROPALLET,P.POSICION_ID)='1'))
								AND((@FLG_CONTROL_CANT_PALLET='0' OR @MP>1)OR(DBO.GUARDADO_PALLET_INGRESO_CANT_PALLET(@CLIENTEID, @PRODUCTOID, @VPOS_EVAL)=1))
								AND ((@FLG_CONTROL_CANT_PALLET='1')OR(P.POSICION_ID NOT IN(	SELECT POSICION_ID FROM SYS_LOCATOR_ING)))
								AND ((@FLG_CONTROL_CANT_PALLET='1')OR(TP.POSICION_ACTUAL IS NULL))
								AND P.POSICION_ID=@VPOS_EVAL 
						UNION ALL
						SELECT 	 NULL AS POSICION_ID
