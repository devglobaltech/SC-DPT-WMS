/****** Object:  StoredProcedure [dbo].[Frontera_FinPicking]    Script Date: 03/18/2016 17:44:19 ******/
...
BEGIN
	...
	From	picking p (NoLock)
			...
			(	select	sum(isnull(cantidad_solicitada,0))cantidad_solicitada, codigo_viaje
				from	sys_int_det_documento ss (nolock) inner join sys_int_documento s (nolock)
	...
	From	picking p (NoLock)
			...
			INNER JOIN (SELECT	DOCUMENTO_ID
						FROM	PICKING P
						WHERE	FIN_PICKING = '2'
						GROUP BY DOCUMENTO_ID
						HAVING COUNT(DISTINCT VIAJE_ID) = 1
			)	AS Z ON (P.DOCUMENTO_ID = Z.DOCUMENTO_ID)			
