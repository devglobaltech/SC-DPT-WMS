...
/****** Object:  StoredProcedure [dbo].[SYS_DEV_BULTOS]    Script Date: 06/06/2014 12:10:23 ******/
	...
	WHILE @@FETCH_STATUS=0
		...
		IF (@DOC_EXT <> '' AND @DOC_EXT IS NOT NULL AND @STATUS='D40') BEGIN
			...
			IF (@TD='I01' AND @ESTADO=3 AND @TC='DO') BEGIN 
				EXEC SYS_DEV_I03 @DOC_EXT=@DOC_EXT ,@ESTADO=1  ,@DOCUMENTO_ID=@DOCUMENTO_ID 
			END --IF
			...
			IF (@TD IS NULL AND @ESTADO=1 AND @TC='IM') BEGIN
				EXEC SYS_DEV_I07 @DOC_EXT=@DOC_EXT ,@ESTADO=@ESTADO  ,@DOCUMENTO_ID=@DOCUMENTO_ID 
			END --IF
		...
		IF (@ESTADO=1 AND NOT (@TD = 'I01' AND @TC = 'DO') AND NOT (@TD = 'I01' AND @TC = 'DE')
			AND NOT (@TD = 'I04' AND @TC = 'PP') AND NOT (@TD = 'I04' AND @TC = 'DE')) BEGIN 
			EXEC SYS_DEV_I08 @DOC_EXT=@DOC_EXT ,@ESTADO=@ESTADO  ,@DOCUMENTO_ID=@DOCUMENTO_ID 
			RETURN
		END -- IF
