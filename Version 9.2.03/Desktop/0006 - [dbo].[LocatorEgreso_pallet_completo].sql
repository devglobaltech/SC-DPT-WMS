/****** Object:  StoredProcedure [dbo].[LocatorEgreso_pallet_completo]    Script Date: 08/19/2014 14:49:58 ******/
			...
			BEGIN
				...
				from	(SELECT	 dd.fecha_vencimiento			,isnull(p.orden_picking,99999) as ORDEN_PICKING		,'POS' as ubicacion
						...
						FROM	rl_det_doc_trans_posicion rl
								...
								inner join posicion p on (rl.posicion_actual=p.posicion_id and isnull(p.bestfit,'0')='1' and isnull(p.picking,'0')='1')
				...
				Begin	
					...
					begin   
						...
						begin
							...
							SELECT @Asignado=SUM(CANTIDAD) FROM det_documento_aux WHERE DOCUMENTO_ID=@pDocumento_id and PRODUCTO_ID=@Producto_id
							...
							SELECT	@PDOCUMENTO_ID,RL.RL_ID,@CLIENTE_ID, @PRODUCTO_ID,RL.CANTIDAD,RL.EST_MERC_ID,RL.CAT_LOG_ID,DD.NRO_BULTO,DD.DESCRIPCION,DD.NRO_BULTO,DD.FECHA_VENCIMIENTO,DD.NRO_DESPACHO,
									DD.NRO_PARTIDA,DD.UNIDAD_ID,DD.TIE_IN,DD.ITEM_OK,DD.CAT_LOG_ID_FINAL,DD.PROP1,DD.PROP2,DD.PROP3,@QTYSOL,DD.NRO_SERIE
							FROM	DET_DOCUMENTO DD INNER JOIN DET_DOCUMENTO_TRANSACCION DDT
									ON(DD.DOCUMENTO_ID=DDT.DOCUMENTO_ID AND DD.NRO_LINEA=DDT.NRO_LINEA_DOC)
									INNER JOIN RL_DET_DOC_TRANS_POSICION RL
									ON(RL.DOC_TRANS_ID=DDT.DOC_TRANS_ID AND RL.NRO_LINEA_TRANS=DDT.NRO_LINEA_TRANS)
