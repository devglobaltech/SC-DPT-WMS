/****** Object:  StoredProcedure [dbo].[DOCUMENTOSEGRESOEXPO]    Script Date: 10/10/2014 16:24:54 ******/
...
CREATE PROCEDURE [dbo].[DOCUMENTOSEGRESOEXPO]
	...
	@TIPO			VARCHAR		OUTPUT,
	@PDELIVERYORDER	VARCHAR(100)OUTPUT
AS	
BEGIN
	...
	IF @TIPO='C' BEGIN
		...
		WHERE	P.CANT_CONFIRMADA>0		
				--AND D.STATUS='D40'
				AND ((@PDELIVERYORDER IS NULL)OR(D.NRO_DESPACHO_IMPORTACION=@PDELIVERYORDER))
	...
	IF @TIPO='D' BEGIN
		...
		SELECT	P.NRO_LINEA * 10																AS [LINE],
				...
				--+ RIGHT('0000'+CONVERT(VARCHAR(4), CAST(D.DOCUMENTO_ID AS VARCHAR)), 4)		AS [3PLPALLETID],
				+ CAST(D.DOCUMENTO_ID AS VARCHAR)												AS [3PLPALLETID],
				...
				CASE WHEN P.NRO_SERIE IS NULL THEN P.CANT_CONFIRMADA ELSE NULL END				AS [QUANTITY],
				NULL																			AS [WEIGHT],
				NULL																			AS [LENGTH],
				NULL																			AS [WIDTH],
				NULL																			AS [HEIGHT],
