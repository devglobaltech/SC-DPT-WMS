...
/****** Object:  StoredProcedure [dbo].[FUNCIONES_INVENTARIO_API#GETDIFINVENTARIO_XDOC]    Script Date: 09/11/2014 15:58:17 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[FUNCIONES_INVENTARIO_API#GETDIFINVENTARIO_XDOC]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[FUNCIONES_INVENTARIO_API#GETDIFINVENTARIO_XDOC]
GO

CREATE PROCEDURE [dbo].[FUNCIONES_INVENTARIO_API#GETDIFINVENTARIO_XDOC]
...
BEGIN
  ...
  DECLARE @INVENTARIO_ID AS NUMERIC(20,0)
  
  SELECT @INVENTARIO_ID = INVENTARIO_ID FROM INVENTARIO WHERE DOC_TRANS_ID = @DOC_TRANS_ID
