/****** Object:  StoredProcedure [dbo].[LocatorEgreso_Produccion]   Script Date: 13/05/2016 10:15:29 a.m. ******/
DROP PROCEDURE [dbo].[LocatorEgreso_Produccion]
GO

CREATE        Procedure [dbo].[LocatorEgreso_Produccion]
...
Begin
  ...
  declare @MSG					as varchar(10)
  declare @CURDOCS				as CURSOR
  ...
  BEGIN
	...
	Begin try --No puedo sacar la transaccion del todo porque sino corro el riesgo de que se lean dos veces el mismo registro.
		...
		Set @RsExist = Cursor For
		  ...
		  from	( SELECT	dd.fecha_vencimiento
				  ...
				  FROM	  rl_det_doc_trans_posicion rl
						  ...
						  inner join posicion p on (rl.posicion_actual=p.posicion_id and p.BESTFIT='1')
						  ...
						  left join consumo_locator_egr con on(rl.RL_ID = con.rl_id)
				  WHERE	  1=1
						  and con.rl_id is null
						  and rl.disponible='1'
						  and isnull(em.disp_egreso,'1')='1'
						  and isnull(em.picking,'1')='1'
						  and p.pos_lockeada='0' and p.picking='1'
						  and cl.disp_egreso='1' 
						  and cl.DISP_PRODUCCION='1'	--Solo para entender si sale para produccion.
		...
		Open @RsExist
		Fetch Next From @RsExist into	@Fecha_Vto,			@OrdenPicking,		@Tipo_Posicion,		@Codigo_Posicion,
										@pCliente_id,		@Producto_id,		@Cantidad,			@vRl_id,
		...
		Begin	
		  ...
		  if (@vQtyResto>0) begin   
			 if (@vQtyResto>=@Cantidad) begin
				...
				values  
				(@DOCIDPIVOT,@vNroLinea,@pCliente_id,@Producto_id,@Cantidad,@EST_MERC_ID,'TRAN_EGR',@NRO_BULTO,@DESC
  ...
  BEGIN
	...
	--esta parte es importante porque se ocupa de splitear lineas
	begin try
		...
		Begin
		  ...
		  if (@Saldo>0) begin
			...
			insert into rl_det_doc_trans_posicion 
					(doc_trans_id,nro_linea_trans,posicion_anterior,posicion_actual,cantidad,tipo_movimiento_id,
