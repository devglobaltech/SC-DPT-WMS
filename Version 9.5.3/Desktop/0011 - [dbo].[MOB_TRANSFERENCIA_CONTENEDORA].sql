/****** Object:  StoredProcedure [dbo].[MOB_TRANSFERENCIA_CONTENEDORA]    Script Date: 11/11/2015 11:22:17 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[MOB_TRANSFERENCIA_CONTENEDORA]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[MOB_TRANSFERENCIA_CONTENEDORA]
GO

CREATE PROCEDURE [dbo].[MOB_TRANSFERENCIA_CONTENEDORA]
	@POSICION_O			AS VARCHAR(45),
	@POSICION_D			AS VARCHAR(45),
	@USUARIO 			AS VARCHAR(30),
	@PALLET_DESTINO		AS VARCHAR(100),
...
BEGIN
	...
	DECLARE @REQ_PALLET		AS CHAR(1)
	
	IF @PALLET_DESTINO IS NULL BEGIN
		SELECT	@REQ_PALLET=ISNULL(PR.PALLET_AUTOMATICO,'0')
		...
		WHERE	DD.NRO_BULTO=@CONTENEDORA
				AND ((P.POSICION_COD=@POSICION_O)OR(N.NAVE_COD=@POSICION_O))
					
		IF @REQ_PALLET='1' BEGIN
			EXEC DBO.GET_VALUE_FOR_SEQUENCE 'NROPALLET_SEQ', @PALLET_DESTINO OUTPUT
		END
	END
	...
	DECLARE CUR_RL_TR CURSOR FOR
		...
		Exec [dbo].[Mob_Transf_VerificaCatLogContenedora] @Contenedora, @Posicion_O, @TRANSFIERE OUTPUT
		If @Transfiere=0
		Begin
			Return
		End
		...
		Exec [Verifica_Picking_Contenedora] @Contenedora, @PICKING OUTPUT
		If @Picking=1
		Begin
		...
		EXEC ACTUALIZA_POS_PICKING_TR @POSICION_O,@POSICION_D,@CONTENEDORA
