/****** Object:  StoredProcedure [dbo].[VERIFICA_EXISTENCIAS_VIAJE]    Script Date: 11/28/2014 10:22:09 ******/
...
BEGIN
	...
	DECLARE @TIPO_DOCUMENTO_ID VARCHAR(50)
	...
	BEGIN TRY
		...
		BEGIN
			...
			BEGIN
				...
				----------------------------------------------------------------------------------------------
				IF (@TIPO_DOCUMENTO_ID='E08')BEGIN
					...
					FROM 	RL_DET_DOC_TRANS_POSICION RL
							...
							INNER JOIN POSICION P					(NOLOCK)	ON(RL.POSICION_ACTUAL=P.POSICION_ID AND P.POS_LOCKEADA='0')
							INNER JOIN NAVE N						(NOLOCK)	ON(P.NAVE_ID=N.NAVE_ID AND ISNULL(N.FLG_GARANTIA,'0')='1')
					...
					FROM	RL_DET_DOC_TRANS_POSICION RL
							...
							INNER JOIN DET_DOCUMENTO DD				(NOLOCK)	ON (DDT.DOCUMENTO_ID=DD.DOCUMENTO_ID AND DDT.NRO_LINEA_DOC=DD.NRO_LINEA)
							INNER JOIN CATEGORIA_LOGICA CL			(NOLOCK)	ON (RL.CLIENTE_ID=CL.CLIENTE_ID AND RL.CAT_LOG_ID=CL.CAT_LOG_ID AND CL.DISP_EGRESO='1' AND CL.PICKING='1')
							INNER JOIN NAVE N						(NOLOCK)	ON (RL.NAVE_ACTUAL=N.NAVE_ID AND N.PRE_EGRESO='0' AND N.PRE_INGRESO='0' AND ISNULL(N.FLG_GARANTIA,'0')='1')
							LEFT JOIN ESTADO_MERCADERIA_RL EM		(NOLOCK)	ON (RL.CLIENTE_ID=EM.CLIENTE_ID AND RL.EST_MERC_ID=EM.EST_MERC_ID) 
					WHERE 	DD.CLIENTE_ID = @CLIENTE_ID 
							AND RL.DOC_TRANS_ID_EGR IS NULL 
							AND RL.NRO_LINEA_TRANS_EGR IS NULL 
