/****** Object:  StoredProcedure [dbo].[SYS_DEV_I01_BULTOS]    Script Date: 12/16/2014 15:46:29 ******/
...
AS    
	...
	DECLARE @V_DOC_EXT		AS VARCHAR(100)
	DECLARE @V_DOCUMENTO_ID AS NUMERIC(20,0)
	DECLARE @V_PRODUCTO_ID	AS VARCHAR(100)
	DECLARE @V_CANTIDAD_REV AS NUMERIC(20,6)
	DECLARE @E_SP			AS VARCHAR(2)
	DECLARE @CLIENTE_ID		AS VARCHAR(40)
...
begin   
	...
	if @qty > 0 BEGIN
		...
		SET @MSG = 'EL DOCUMENTO NRO. ' + CAST(@documento_id AS VARCHAR) + ' YA ESTA INFORMADO AL ERP, POR FAVOR NOTIFIQUE A SISTEMAS'
		raiserror(@MSG,15,1)
	END
    ...
    if @e_sp='1' begin        
		...
		BEGIN TRY
			exec dbo.ERP_INGRESO @cliente_id, @doc_ext 
			
			INSERT INTO AUDITORIA_ERP_INGRESOS VALUES(GETDATE(),@CLIENTE_ID,@doc_ext,'OK','ADMIN',HOST_NAME(),'MOBILE',NULL,@DOCUMENTO_ID);
			
		END TRY
		BEGIN CATCH
		
			INSERT INTO AUDITORIA_ERP_INGRESOS VALUES(GETDATE(),@CLIENTE_ID,@doc_ext,'ERROR','ADMIN',HOST_NAME(),'MOBILE',ERROR_MESSAGE(),@DOCUMENTO_ID);						
